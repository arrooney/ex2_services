...
SAT_returnState time_management_app(csp_packet_t *packet) {
  ...
  switch (ser_subtype) {
    case SET_TIME:
      ...
      temp_time.unix_timestamp = csp_letoh32(temp_time.unix_timestamp); // buffer data is little endian and this will swap it
      if (!TIMESTAMP_ISOK((uint32_t)temp_time.unix_timestamp)) {
        return SATR_ERROR;
      }
    ...
    case GET_TIME:
      ...
      packet->length = sizeof(uint32_t) + 1; // plus one for sub-service
